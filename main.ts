...
namespace robobloq {
    ...
    export class Robot {
        ...
        write(list:number[]):void{
            ...
            // 调试
            let list2:number[]=[82,66,11,0,19,250,1,6,1,244,0];
            const pro = new Protocol();
            list2[11-1] = pro.sumCheck(list2,0);

            let buffer2 = pins.createBuffer(11);
            for(let i:number=0; i<list2.length; i++){
                buffer2.setNumber(NumberFormat.Int8LE, i, list2[i]);
            }
